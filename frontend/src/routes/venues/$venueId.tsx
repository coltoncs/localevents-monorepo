...
import { BookingRequestForm } from "#/components/music/BookingRequestForm";
...
function VenuePage() {
	...
	return (
		<div className="mx-auto max-w-7xl space-y-4 px-4 py-6 sm:px-6 lg:px-8">
			...
			{!editing && venue?.AcceptsBookingRequests && (
				<BookingRequestForm venueId={venue.ID} venueName={venue.VenueName} />
			)}
			...
			{isLoading ? (
				...
				<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
					{events.map((event) => {
						const isSaved =
							savedEvents?.length &&
							savedEvents.find((savedEvent) => event.ID === savedEvent.ID);
						return (
