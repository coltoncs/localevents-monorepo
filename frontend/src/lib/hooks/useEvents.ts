...
export function useSeriesEvents(seriesId: string | undefined) {
	return useQuery({
		queryKey: queryKeys.events.series(seriesId ?? ""),
		queryFn: () => apiClient<Event[]>(`/api/events/series/${seriesId}`),
		enabled: !!seriesId,
	});
}
...
export function useUpdateSeries() {
	...
	return useMutation({
		mutationFn: ({
			...
			seriesId: string;
			data: Omit<CreateEventInput, "start_time">;
		}) =>
			apiClient<Event[]>(`/api/events/series/${seriesId}`, {
				method: "PUT",
				body: JSON.stringify(data),
			}),
		onSuccess: () => {
