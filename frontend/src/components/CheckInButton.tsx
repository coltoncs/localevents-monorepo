...
import {
	useBeverageCheckInCounts,
	useCheckIn,
	useMyCheckInStatus,
} from "#/lib/hooks/useBeverageCheckIns";

export function CheckInButton({ beverageId }: { beverageId: string }) {
	const { isSignedIn } = useAuth();
	const { openSignIn } = useClerk();
	const checkIn = useCheckIn(beverageId);
	const { data: status } = useMyCheckInStatus(beverageId, !!isSignedIn);
	const { data: counts } = useBeverageCheckInCounts(beverageId);
