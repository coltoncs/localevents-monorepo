...
export function SuggestFoodEditModal({
	food,
	onClose,
}: {
	food: Food;
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		if (name !== food.Name) changes.name = name;
		if (cuisine !== food.Cuisine) changes.cuisine = cuisine;
		if (address !== (food.Address ?? "")) changes.address = address;
		if (city !== (food.City ?? "")) changes.city = city;
		if (state !== (food.State ?? "")) changes.state = state;
		if (zip !== (food.Zip ?? "")) changes.zip = zip;
		if (phone !== (food.Phone ?? "")) changes.phone = phone;
		if (website !== (food.Website ?? "")) changes.website = website;
		if (hours !== (food.Hours ?? "")) changes.hours = hours;
